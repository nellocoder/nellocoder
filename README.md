<p align="center">
  <img src="https://komarev.com/ghpvc/?username=nellocoder&color=2ea44f&style=flat" alt="Profile views" />
</p>

<h1 align="center">👋 Kishoyian Duncan</h1>

<p align="center"><b>🔬 Research Analyst · 📊 Data Scientist · 📈 Statistician</b></p>

<p align="center">Research analyst and data scientist with expertise in statistical modelling, ML pipelines, and Monitoring, Evaluation, and Learning (MEL) frameworks for evidence-based decision making.</p>

<p align="center"><i>Translating complex data into actionable insights for policy, public health, and operational decision-making.</i></p>

<p align="center">
  <a href="mailto:kishsaningo@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white" /></a>
  <a href="https://linkedin.com/in/duncan-kishoyian"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white" /></a>
  <a href="https://x.com/nellosynn"><img src="https://img.shields.io/badge/X-000000?logo=x&logoColor=white" /></a>
  <img src="https://img.shields.io/badge/Nairobi,%20Kenya-2ea44f" />
</p>

---

<p align="center">
  <img src="https://streak-stats.demolab.com?user=nellocoder&theme=tokyonight&hide_border=true" height="165" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=nellocoder&layout=compact&theme=tokyonight&hide_border=true&cache_seconds=1800" height="165" />
</p>

---

## 🚀 Featured Projects

### ✈️ [Flight Delay Prediction](https://github.com/nellocoder/flight-delay-prediction)
> Regression pipeline predicting airline delay minutes from flight volume and carrier data.

| Metric | Result |
|--------|--------|
| **R² Score** | ~0.95 on monthly aggregates |
| **Key Insight** | Carrier identity explains ~30% of delay variance beyond traffic volume |
| **Stack** | Python · Pandas · Scikit-learn · Seaborn · Jupyter |

---

### 🩺 [Diabetes Prediction Analysis](https://github.com/nellocoder/diabetes-prediction-analysis)
> End-to-end ML workflow for diabetes risk prediction using the Pima Indians dataset.

| Metric | Result |
|--------|--------|
| **Accuracy** | ~75% with Logistic Regression |
| **Segmentation** | K-Means (k=3) identifies distinct metabolic-risk patient profiles |
| **Stack** | Python · Pandas · Scikit-learn · Matplotlib |

---

### 📊 [Mathematical Modeling of Alcoholism](https://github.com/nellocoder/Mathematical-Modeling-of-Alcoholism)
> Compartmental ODE model evaluating educational campaign impact on alcohol-use population dynamics.

| Metric | Result |
|--------|--------|
| **Model Type** | SIR-style compartmental ODE |
| **Output** | Equilibrium behaviour & sensitivity analysis via phase-plane visualisation |
| **Stack** | Python · SciPy · NumPy · Matplotlib |

---

### 🚔 [Crime Analysis](https://github.com/nellocoder/Crime_analysis)
> Statistical and network-based analysis of crime patterns for intelligence-led decision making.

| Metric | Result |
|--------|--------|
| **Method** | Network centrality & exploratory spatial analysis |
| **Insight** | Identifies core distribution nodes and brokerage roles in urban networks |
| **Stack** | Python · NetworkX · Pandas · Matplotlib |

---

## 📌 Portfolio Standards

All featured repositories are built to these standards:

- ✅ **Reproducible** — Every repo includes `requirements.txt` and step-by-step execution instructions
- ✅ **Documented** — Methodologies, assumptions, and limitations are explicitly stated
- ✅ **Licensed** — Open-source licenses for reuse and collaboration
- ✅ **Decision-focused** — Outputs designed for policy-makers and operational teams, not just technical peers

---

## 🧠 Core Areas

- 📊 **Statistical Modelling** — Inference, regression, and experimental design
- 🤖 **Machine Learning** — End-to-end pipelines for regression, classification, and clustering
- 🏥 **Public Health Research** — Epidemiological analysis and intervention modelling
- 🗺️ **Crime & Geospatial Analysis** — Pattern detection, network analysis, and spatial statistics
- 📋 **MEL Systems** — Monitoring, Evaluation, and Learning framework design and implementation
- 🔬 **Mathematical Modelling** — Compartmental ODEs for assessing public health intervention impact

---

## 🛠️ Tech Stack

| Category | Tools |
|----------|-------|
| **Languages** | Python · R · SQL · TypeScript |
| **Data & ML** | Pandas · Scikit-learn · Statsmodels · NumPy |
| **Visualization** | Matplotlib · Seaborn · Power BI · ggplot2 |
| **Data Collection** | KoboToolbox · REDCap · ODK · SPSS · STATA |
| **Workflows** | Git · Jupyter · RMarkdown · GitHub Actions |

---

## 📍 About

Based in **Nairobi, Kenya** 🇰🇪

Work focuses on applying statistical and machine learning methods to **public health, crime analysis, and decision-support systems**.

> **Principles:** Reproducibility · Interpretability · Policy-relevant insights

Contributions and replications are welcome. All flagship repositories include open-source licenses and end-to-end execution instructions.

---

## 👨‍🏫 Teaching & Academic Contributions

- 📚 Developed instructional materials for undergraduate mathematics (Algebra, Calculus)
- 📝 Produced LaTeX-based academic and technical documents
- 🎓 Supported training in data collection tools (KoboToolbox, ODK) and reproducible analysis workflows

---

## 🔍 Currently Working On

- 🗺️ Crime pattern analysis using geospatial and statistical methods
- 🔁 Strengthening reproducible research workflows with CI/CD for Jupyter notebooks
- 📊 Building MEL dashboards for public health programme evaluation

---

## 🤝 Open To

- Research collaborations in public health, epidemiology, and social policy
- MEL system design, implementation, and third-party evaluation
- Data science and analytics consultancy for NGOs and government agencies
- Academic partnerships, peer review, and teaching opportunities

---

<h2 align="center">📫 Let's Connect</h2>

<p align="center">
  <a href="mailto:kishsaningo@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white&style=for-the-badge" /></a>
  <a href="https://linkedin.com/in/duncan-kishoyian"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white&style=for-the-badge" /></a>
  <a href="https://x.com/nellosynn"><img src="https://img.shields.io/badge/X-000000?logo=x&logoColor=white&style=for-the-badge" /></a>
</p>

<p align="center"><em>Open to research collaborations, MEL consultancy, and data-driven projects.</em></p>
